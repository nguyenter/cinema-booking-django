...
def movie_poster_upload_path(instance, filename):
    """Generate unique filename for movie poster"""
    ext = filename.split('.')[-1]
    # Use movie id if exists, otherwise use title
    if instance.id:
        filename = f'movie_{instance.id}_{instance.title.replace(" ", "_")}.{ext}'
    else:
        filename = f'movie_{instance.title.replace(" ", "_")}.{ext}'
    return os.path.join('movie_poster', filename)
...
class Movie(models.Model):
    ...
    poster = models.ImageField(_('Ảnh bìa'), upload_to=movie_poster_upload_path, null=False, blank=False)
...
class Showtime(models.Model):
    """Khung giờ chiếu cho phim"""
    id = models.AutoField(_('Mã suất chiếu'), primary_key=True)
    movie = models.ForeignKey(Movie, on_delete=models.CASCADE, verbose_name=_('Phim'), related_name='showtimes')
    room = models.ForeignKey(Room, on_delete=models.CASCADE, verbose_name=_('Phòng chiếu'))
    start_time = models.DateTimeField(_('Thời gian bắt đầu'), null=False, blank=False)
    end_time = models.DateTimeField(_('Thời gian kết thúc'), null=True, blank=True)
    is_active = models.BooleanField(_('Đang hoạt động'), default=True, null=False, blank=False)
    
    def __str__(self):
        return f"{self.movie.title} - {self.room.name} - {self.start_time.strftime('%d/%m/%Y %H:%M')}"
    
    def get_available_seats_count(self):
        """Lấy số ghế còn trống cho suất chiếu này"""
        total_seats = self.room.capacity
        booked_seats = Ticket.objects.filter(showtime=self).count()
        return total_seats - booked_seats
    
    def get_booked_seats_count(self):
        """Lấy số ghế đã đặt cho suất chiếu này"""
        return Ticket.objects.filter(showtime=self).count()
    
    class Meta:
        verbose_name = _('Suất chiếu')
        verbose_name_plural = _('Suất chiếu')
        ordering = ['start_time']
        unique_together = [['movie', 'room', 'start_time']]  # Tránh trùng suất chiếu
...
class Ticket(models.Model):
    ...
    showtime = models.ForeignKey(Showtime, on_delete=models.CASCADE, verbose_name=_('Suất chiếu'), null=True, blank=True)
    ...
    date_time = models.DateTimeField(_('Ngày giờ chiếu'), null=True, blank=True, help_text=_('Giữ lại để tương thích với dữ liệu cũ'))
...
# Update the seat status when a ticket is created or deleted
@receiver(post_save, sender=Ticket)
def update_seat_status_on_create(sender, instance, created, **kwargs):
    if created:
        Seat.objects.filter(id=instance.seat.id).update(is_available=False)

@receiver(post_delete, sender=Ticket)
def update_seat_status_on_delete(sender, instance, **kwargs):
    # Check if there are any other tickets for this seat at the same date_time
    remaining_tickets = Ticket.objects.filter(seat=instance.seat, date_time=instance.date_time).exists()
    if not remaining_tickets:
        Seat.objects.filter(id=instance.seat.id).update(is_available=True)
